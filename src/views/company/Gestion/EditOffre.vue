<template>
    <main class="flex  ">
        ...
        <section class=" w-[80%] m-auto rounded-lg p-4 shadow-lg  bg-white bg-opacity-20 ">
            <form @submit.prevent="updatePost(form)" method="put"
                ...
                <input type="text" v-model="form.title" class="w-[70%] py-2 rounded" name="title"
                ...
                <input type="text" v-model="form.description" name="description"
                    class="w-[70%] h-20 py-2 rounded items-start" placeholder="a little description">
                <div v-if="errors.description">
                    <span class="text-red-500">{{ errors.description }}</span>
                </div>
                <label for="tag" class="text-black font-semibold">Tag</label>
                <input type="text" v-model="form.tag" name="tag" class="w-[70%] py-2 rounded"
                    placeholder="Put The tags of the poste">
                <div v-if="errors.tag">
                    <span class="text-red-500">{{ errors.tag }}</span>
                </div>
                <label for="city" class="text-black font-semibold">City</label>
                <input type="text" v-model="form.city" name="city" class="w-[70%] py-2 rounded"
                ...
                <input type="text" v-model="form.type_of_post" name="type_of_post" class="w-[70%] py-2 rounded"
...
const updatePost = async () => {
    ...
    axios.put("http://127.0.0.1:8000/api/V1/posts/" + id, form)
        ...
        .catch((err) => {
            ...
            errors.type_of_post = err.response.data.errors.type_of_post;
